...
backup_project() {
    ...
    if [ -d "$source_dir" ]; then
        ...
        rsync -av --timeout=60 "$source_dir" "$BACKUP_DIR" 2>&1 | tee ~/Desktop/rsync_output.log
        
        # Check rsync exit status
        if [ ${PIPESTATUS[0]} -eq 0 ]; then
            echo "  Backup of $project completed successfully."
        else
            echo "  Errors occurred during backup of $project. Check /tmp/rsync_output.log for details."
...
echo "Please check /tmp/rsync_output.log for any error messages."